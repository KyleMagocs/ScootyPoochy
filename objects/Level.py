...
class Level:
    ...
    def build_level(self, my_doors, my_rooms):
        ...
        for room, door_x in zip(my_rooms, my_doors):
            ...
            for y in range(height+125, height + _room.height + 125, floor_image.get_height()):
...
class ButtLevel(Level):
    ...
    def __init__(self):
        from objects.Room import Room_Finish, Room_One, Room_Start, Room_Two, Room_Three, Bathroom
        super().__init__()
        self.theme = TempTheme()
        self.width = self.theme.background_sprite.get_width()

        my_rooms = Room_Finish, Room_Three, Room_One, Room_Two, Bathroom, Room_Three, Room_Start
